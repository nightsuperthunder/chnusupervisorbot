...
def cllogch(message: Message):
    ...
    if int(message.from_user.id) == int(constants.bossid):
        ver = bot.reply_to(message, 'Очистити логи?')
...
def cllog (message: Message):
    ...
    if ver == 'так':
        ...
        bot.send_message(message.chat.id, 'Невдала спроба очищення')


@bot.message_handler(commands=['logsend'])
def sendlog (message : Message):
